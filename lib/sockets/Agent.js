...
var Elapsed = Common.elapsed;

var FileHelper = require('corci-libs').FileHelper;
...
Agent.prototype.onBuildSuccess = function (responseBuild) {
    ...
    fs.mkdirs(locationPath, function (err) {
        if (err) {
            ...
            FileHelper.writeFiles(locationPath, outputFiles, 'the cordova build agent worker output files on {0} [a]'.format(build.conf.platform), true, function (err) {
                if (err) {
                    FileHelper.freeMemFiles(build.outputFiles);
                    ...
                    FileHelper.freeMemFiles(build.outputFiles);
                    FileHelper.cleanLastFolders(server.conf.keep, server.location + "/*", function (err, stats) {
                        if (err) {
...
Agent.prototype.startBuild = function (build) {
    ...
    FileHelper.readFiles(files, '[AGENT.startBuild] the cordova build server\n', function (err) {
        try {
            if (err) {
